...
export const generateBlockchainVerifiedLinks = async (
  ...
  try {
    ...
    const verificationUrl = `${window.location.origin}/blockchain-verify/${blockchainSession.blockchain_tx_id}`;
...
export const generateSecurePortalAccess = async (
  ...
  try {
    ...
    const portalUrl = `${window.location.origin}/customer-portal/${accessToken}`;
